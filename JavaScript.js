$(document).ready(function () {
  ...
  $("#signUpForm").on("submit", function (e) {
    ...
    setTimeout(function () {
      form[0].reset();
      form.find(".signup-success").fadeOut(400, function () {
        $(this).remove();
      });
      $("#signUpBox").hide();
      updateUI();
  ...
  $("#signOutBtn").on("click", function (e) {
    ...
    var $btn = $("#signInBtn");
    $btn.text("Signed out").css({
      "pointer-events": "none",
      "color": "green"
    });

    setTimeout(function () {
      $btn.text("Sign in").css({
        "pointer-events": "auto",
        "color": ""
      });
      updateUI();
  ...
  $(".loan-form").on("submit", function (e) {
    ...
    form.find("input[required]").each(function () {
      ...
      if (value === "") {
        ...
        if (ph.includes("name") && !nameRegex.test(value)) {
          fieldValid = false;
        }
        ...
        if (input.hasClass("money-input") || type === "number") {
          ...
          if (!numberRegex.test(raw) || Number(raw) <= 0) {
            fieldValid = false;
          }
        }
